import 'package:flutter/material.dart';

class AppLocalizations {
  AppLocalizations(this.locale);
  final Locale locale;

  static AppLocalizations of(BuildContext context) {
    return Localizations.of<AppLocalizations>(context, AppLocalizations)!;
  }

  static const LocalizationsDelegate<AppLocalizations> delegate =
      _Delegate();

  static const supportedLocales = [Locale('ja'), Locale('en')];

  static const _strings = <String, Map<String, String>>{
    'ja': {
      'app_name': 'かがみ',
      // MirrorScreen
      'mirror': 'ミラー',
      'others_view': '他人から',
      'divining': 'メッセージを準備中...',
      'fortune_done': 'メッセージ届いた！',
      'outfit': '着せ替え',
      // FortuneResultScreen
      'love': 'ときめき',
      'money': 'エネルギー',
      'work': '集中力',
      'rabbit_message': 'ウサギからのひとこと',
      'back_to_mirror': '鏡に戻る',
      'watch_ad': 'もう一度',
      'watch_ad_unavailable': '',
      // CostumeScreen
      'outfits_title': 'きせかえ',
      'outfits_subtitle': 'あなたのウサギをおしゃれに',
      'back': 'もどる',
      'coming_soon': 'Coming soon',
      // Costume names
      'costume_normal': 'ノーマル',
      'costume_carrot': '人参コスプレ',
      'costume_dress': 'ドレス',
      'costume_hiphop': 'ヒップホップ',
      // Menu
      'menu_premium': 'プレミアム',
      'menu_premium_desc': 'メッセージ無制限',
      'menu_premium_unavailable': '購入情報を取得できませんでした。しばらくしてからお試しください',
      'menu_language': '言語',
      'lang_ja': '日本語',
      'lang_en': 'English',
      // IAP
      'plan_name': '無制限プラン',
      'plan_tagline': '何度でも楽しめる',
      'plan_cta': 'もっと楽しむ',
      'plan_cta_sub': '無制限プランで何度でも',
      'redivine': 'もう一度',
      'premium_subscribe': '無制限プランに登録',
      'premium_restore': '購入を復元',
      'premium_loading': '処理中...',
      'premium_already': '無制限プラン加入中',
      'premium_active_badge': '無制限プラン',
      'per_month': '/月',
      'subscription_monthly': '月額自動更新',
      'subscription_links': '利用規約 ・ プライバシーポリシー',
      'subscription_benefit': '無制限プランに登録すると何度でもメッセージが楽しめます。',
      'subscription_manage': '購入の管理・解除はApple IDのアカウント設定から行えます。',
      'subscription_cancel': '契約期間終了の24時間前までに解約すると、次の期間の請求が発生しません。',
      // Legal
      'menu_support': 'サポート',
      'menu_terms': '利用規約',
      'menu_privacy': 'プライバシーポリシー',
    },
    'en': {
      'app_name': 'Mirror',
      'mirror': 'Mirror',
      'others_view': "Others' View",
      'divining': 'Preparing your message...',
      'fortune_done': 'Message Ready!',
      'outfit': 'Outfits',
      'love': 'Spark',
      'money': 'Energy',
      'work': 'Focus',
      'rabbit_message': "Rabbit's Message",
      'back_to_mirror': 'Back to Mirror',
      'watch_ad': 'Try Again',
      'watch_ad_unavailable': '',
      'outfits_title': 'Outfits',
      'outfits_subtitle': 'Dress up your rabbit!',
      'back': 'Back',
      'coming_soon': 'Coming soon',
      'costume_normal': 'Normal',
      'costume_carrot': 'Carrot',
      'costume_dress': 'Dress',
      'costume_hiphop': 'Hiphop',
      'menu_premium': 'Premium',
      'menu_premium_desc': 'Unlimited messages',
      'menu_premium_unavailable': 'Could not load purchase info. Please try again later.',
      'menu_language': 'Language',
      'lang_ja': '日本語',
      'lang_en': 'English',
      // IAP
      'plan_name': 'Unlimited Plan',
      'plan_tagline': 'Unlimited messages',
      'plan_cta': 'Unlock More',
      'plan_cta_sub': 'Unlimited messages anytime',
      'redivine': 'Try Again',
      'premium_subscribe': 'Subscribe to Unlimited Plan',
      'premium_restore': 'Restore Purchases',
      'premium_loading': 'Processing...',
      'premium_already': 'Unlimited Plan Active',
      'premium_active_badge': 'Unlimited Plan',
      'per_month': '/month',
      'subscription_monthly': 'Monthly, auto-renews',
      'subscription_links': 'Terms of Service ・ Privacy Policy',
      'subscription_benefit': 'Subscribe to enjoy unlimited messages.',
      'subscription_manage': 'Manage or cancel your subscription in Apple ID account settings.',
      'subscription_cancel': 'Cancel at least 24 hours before the end of the period to avoid being charged for the next period.',
      // Legal
      'menu_support': 'Support',
      'menu_terms': 'Terms of Service',
      'menu_privacy': 'Privacy Policy',
    },
  };

  String get(String key) =>
      _strings[locale.languageCode]?[key] ?? _strings['ja']![key] ?? key;

  String daysLeft(int n) {
    if (locale.languageCode == 'en') return '$n days left';
    return 'あと$n日';
  }
}

class _Delegate extends LocalizationsDelegate<AppLocalizations> {
  const _Delegate();

  @override
  bool isSupported(Locale locale) =>
      ['ja', 'en'].contains(locale.languageCode);

  @override
  Future<AppLocalizations> load(Locale locale) async =>
      AppLocalizations(locale);

  @override
  bool shouldReload(_Delegate old) => false;
}
